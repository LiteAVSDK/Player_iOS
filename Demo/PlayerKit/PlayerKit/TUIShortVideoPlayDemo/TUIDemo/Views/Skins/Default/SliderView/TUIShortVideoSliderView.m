...
-(void)onSeekDragging:(UISlider *)slider {
    if (self.delegate && [self.delegate respondsToSelector:@selector(onSeekDragging:)]) {
        [self.delegate onSeekDragging:slider];
    }
}
...
- (UISlider *)slider {
    if (!_slider) {
        ...
        [_slider addTarget:self action:@selector(onSeekDragging:) forControlEvents:UIControlEventTouchDragInside];
    }
    return _slider;
}
