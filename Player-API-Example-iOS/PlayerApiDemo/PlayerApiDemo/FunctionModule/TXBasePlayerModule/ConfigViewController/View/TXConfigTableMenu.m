...
+ (instancetype)title:(NSString *)title array:(NSArray *)array {
    TXConfigTableMenu *menu = [[TXConfigTableMenu alloc] initWithFrame:CGRectMake(0, 0, TX_SCREEN_WIDTH, TX_SCREEN_HEIGHT)
                                                         title:title
                                                     withArray:array];
    return menu;
...
- (void)setup {
    ...
    //内容view
    _heightMenu = TX_SCREEN_WIDTH / 2.0;
    _heightMenu = 30 + 45 + 40 * [_arr count];
    if (_heightMenu > TX_SCREEN_HEIGHT / 2.0) {
        _heightMenu = TX_SCREEN_HEIGHT / 2.0;
    }
    
    _menu = [[UIView alloc] initWithFrame:CGRectMake(0, TX_SCREEN_HEIGHT - _heightMenu, TX_SCREEN_WIDTH, _heightMenu)];
    _menu.backgroundColor = [UIColor whiteColor];
    
    //标题
    _labTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, TX_SCREEN_WIDTH, 30)];
    ...
    //底部 取消
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, _heightMenu - 45, TX_SCREEN_WIDTH, 45)];
    _bottomView.backgroundColor = [UIColor blackColor];
    
    _btnCancel = [[UIButton alloc] initWithFrame:CGRectMake(0, 5, TX_SCREEN_WIDTH, 40)];
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 30, TX_SCREEN_WIDTH, hTable)];
